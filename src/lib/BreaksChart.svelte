<script>
  import { format, numToString } from "./utils";
...
<div class="container" style="height: {height}px">
	{#each breaks.slice(1) as brk, i}
		...
		<div class="tick" style="left: {i * (100 / (breaks.length - 1))}%; transform: translateX({i == 0 && snapTicks ? '-2px' : '-50%'});">{numToString(breaks[i], dp)}</div>
	{/each}
	<div class="line" style="right: 0;"/>
	<div class="tick" style="right: 0; transform: translateX({snapTicks ? '2px' : '50%'});">{numToString(breaks[breaks.length - 1], dp)}</div>
	{#if selected}
	<div class="marker" style="width: {lineWidth}px; left: calc({pos(selected, breaks)}% - {lineWidth / 2}px);"/>
	<div class="value" style="left: {pos(selected, breaks)}%">{format(selected, unit)}</div>
  {/if}
  {#if hovered}
	<div class="marker marker-hovered" style="width: {lineWidth}px; left: calc({pos(hovered, breaks)}% - {lineWidth / 2}px);"/>
	<div class="value" style="left: {pos(hovered, breaks)}%">{format(hovered, unit)}</div>
  {/if}
</div>
